    ...
    <div className={`bg-input p-4 rounded-lg border ${isPastExam ? 'border-gray-500 opacity-80' : 'border-border'} transition-all hover:border-primary`}>
      <div className="flex justify-between items-start gap-2">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <button
            onClick={() => onEdit(exam)}
            className="px-3 py-1.5 bg-primary/20 text-primary rounded-md hover:bg-primary/30 transition-colors text-sm cursor-pointer"
          >
            Edit
          </button>
          <button
            onClick={() => onDelete(exam.id)}
            className="px-3 py-1.5 bg-destructive/20 text-destructive rounded-md hover:bg-destructive/30 transition-colors text-sm cursor-pointer"
          >
            Delete
          </button>
        </div>
