    ...
    <div className="space-y-4">
      {timeBlocks.map((block) => (
        <div key={block} className="flex flex-col md:flex-row md:items-center md:space-x-4">
          <label className="font-semibold mb-1 md:mb-0 md:w-32 text-gray-700">{block}</label>
          <div className="flex-1 flex space-x-2">
            <input
              type="time"
              value={blockTimes[block]?.startTime || ''}
              onChange={(e) => handleTimeChange(block, 'startTime', e.target.value)}
              className="input w-full box-border text-black border border-gray-300 rounded-md p-2"
            />
